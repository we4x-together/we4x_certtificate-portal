...
export const findCertificateByEmail = async (email: string): Promise<CertificateData | null> => {
  ...
  const match = rows.slice(1).find((row: any[]) => row[0]?.toLowerCase() === email.toLowerCase());
